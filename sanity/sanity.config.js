...
export default defineConfig({
  ...
  plugins: [structureTool({ structure: (S) => S.list().title('Content').items([
    ...
    S.divider(), S.documentTypeListItem('character').title('Characters'), S.documentTypeListItem('episode').title('Episodes & Videos'), S.documentTypeListItem('book').title('Books'), S.documentTypeListItem('blogPost').title('Blog Posts'),

...
const C = {
  ...
  cardBg: '#161616',
  border: '#2a2a2a',
  teal: process.env.EMAIL_ACCENT || '#00c2d4',
...
async function sendMerchantEmail(session, lineItems, status, printfulOrderId) {
  ...
  const subject = `${failed ? '\u26A0 ACTION NEEDED \u2014 ' : ''}New Biker Babies order #${ref} \u2014 ${gbp(session.amount_total)}`;
...
async function saveOrder(session, lineItems, status, printfulOrderId) {
  ...
  const items = (lineItems.data || []).map((li, i) => ({
    ...
    colour: meta(li, 'bikerbabies_colour') || '',
    size: meta(li, 'bikerbabies_size') || '',
